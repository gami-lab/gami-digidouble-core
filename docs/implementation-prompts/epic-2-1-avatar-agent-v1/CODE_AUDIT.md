...
| **Director–Actor model**                             | ✅ The Avatar answers the user directly. The `TODO(EPIC-4.1)` placeholder is in the correct position after the response is built — GM trigger will be non-blocking from that point too.                                                               |
