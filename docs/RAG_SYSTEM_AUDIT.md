...
is applied to persisted chunks without overlap. Oversized normal
...
| Chunking                   | Overlapping windows (~10–20%) or sentence-aware splitting with a hard max chunk size                                                                                                        | Paragraph-aware packing with sentence/raw splitting and an 8,000-character hard cap, without overlap                                                                      | Deterministic paragraph boundaries and oversized-input safety are now covered; overlap is intentionally omitted for this strategy.                                                  |
...
**Addressed — Cap oversized paragraphs.** `toChunkSeeds` applies the 8,000-character hard maximum
without overlap to both ingestion and staged reindex paths. Remaining retrieval-quality work is tracked
below and should be measured against the committed baseline harness.
