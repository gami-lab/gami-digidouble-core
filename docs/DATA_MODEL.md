...
| `gm_states`     | Lightweight persisted Game Master state                | `session_id`, `current_avatar_id`, `progression`, `topics_covered`, `interaction_count`, `next_turn_orchestration`, `updated_at`        | One row per session. `next_turn_orchestration` is replaced after GM output and marked consumed after the matching next Avatar turn. |
