...
| `sessions`      | Durable user run container for one scenario            | `id`, `user_id`, `scenario_id`, `active_avatar_id`, `unlocked_avatar_ids`, `avatar_options`, `gm_notes`, `memory_summary`, `status`, lifecycle timestamps | `avatar_options` stores session-scoped Avatar retrieval experiments. `memory_summary` is a compatibility mirror, not canonical working memory.                                                                                                                                  |
