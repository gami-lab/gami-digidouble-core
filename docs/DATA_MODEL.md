...
| `gm_states`     | Lightweight persisted Game Master state                | `session_id`, `progression`, `interaction_count`, `next_turn_orchestration`, `updated_at`                                                                                   | One row per session. The session owns the active Avatar and memory compaction owns covered topics; `current_avatar_id` and `topics_covered` are not part of the canonical schema.                                                                                           |
...
into conversational memory. The fresh canonical schema rejects `memory`; the existing audit,
migration, and quarantine surface remains intentionally deferred to EPIC 10.1 Prompt 2.
