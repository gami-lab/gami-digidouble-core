...
describe('semantic judge', () => {
  ...
  it('uses the authenticated raw exchange boundary and requests the declared judge model', async () => {
    ...
    expect(body.systemPrompt).toContain('Accept accurate paraphrases')
    expect(body.systemPrompt).toContain('independent acceptance paths, not cumulative requirements')
    expect(body.systemPrompt).toContain('All requiredFacts is sufficient')
    expect(body.systemPrompt).toContain('any one acceptedAlternative is sufficient')
    expect(body.systemPrompt).toContain('Accept compatible additional detail by default')
    expect(body.systemPrompt).toContain('“in the abandoned hotel”')
    expect(body.systemPrompt).toContain('asserts a forbidden claim')
