...
## Regenerate the before baseline
...
with 16 dimensions) and refuses a different dimension:

```sh
pnpm --filter @gami/core retrieval-quality -- \
  --live \
  --output apps/core/src/tools/retrieval-quality/baseline-before.json
```

The command loads the repository `.env` when present, while an already-exported
`OPENAI_API_KEY` takes precedence. The committed report contains profile identity, scores, counts,
