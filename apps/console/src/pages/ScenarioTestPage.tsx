...
async function loadScenarioAvatars(scenarioId: string, setState: SetScenarioTestState): Promise<void> {
  try {
...
async function startSessionFlow(
  ...
  setState((prev) => ({ ...withSessionStarted(prev, session, avatarIds(available.avatars)), conversations }))
...
async function openConversationFlow(conversationId: string, setState: SetScenarioTestState): Promise<void> {
...
function useScenarioDerivedData(state: ScenarioTestState): {
  ...
  const allAvatarsById = useMemo(() => new Map(state.allScenarioAvatars.map((a) => [a.avatarId, a])), [state.allScenarioAvatars])
  const availabilityEntries = useMemo(() => deriveAvatarAvailabilityEntries(state), [state])
  const timelineEntries = useMemo(() => deriveConversationTimeline(state, allAvatarsById), [state, allAvatarsById])
  const selectedMessages = useMemo(() => (state.selectedConversationId ? state.messagesByConversationId[state.selectedConversationId] ?? [] : []), [state.selectedConversationId, state.messagesByConversationId])
  const selectedConversation = useMemo(() => (state.selectedConversationId ? state.conversations.find((c) => c.conversationId === state.selectedConversationId) ?? null : null), [state.selectedConversationId, state.conversations])
  const availableAvatarsForInspector = useMemo(() => state.allScenarioAvatars.filter((a) => state.availableAvatarIds.includes(a.avatarId)), [state.allScenarioAvatars, state.availableAvatarIds])
  return { allAvatarsById, availabilityEntries, timelineEntries, selectedMessages, selectedConversation, availableAvatarsForInspector }
}
...
function useAuxiliaryHandlers(
  ...
  const handleSendDraft = useCallback((event: SyntheticEvent<HTMLFormElement>): void => {
  ...
  }, [handleSwitchAvatar, setState, state.allScenarioAvatars, state.availableAvatarIds, state.session])
...
export function ScenarioTestPage({ scenario }: ScenarioTestPageProps): JSX.Element {
  ...
  const handleSwitchAvatar = useCallback((avatarId: string): void => {
  ...
  const handleSendMessage = useCallback((content: string): void => {
    ...
    void (async () => {
      try {
        await sendMessageFlow(conversationId, content, sessionId, setState, turnIndexRef, allAvatarsById)
      } catch (error) {
  ...
  const handleOpenConversation = useCallback((conversation: ConversationSummary): void => {
    ...
    void (async () => {
      try {
        ...
        setState((prev) => withError(prev, formatApiError(error, 'Failed to load conversation history')))
      } finally {
